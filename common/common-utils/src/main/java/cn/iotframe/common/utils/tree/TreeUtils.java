...
public class TreeUtils {
     ...
     */
    public static <T> List<T> recursive(List<T> list, String idName, String parentIdName, String childrenName) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        List<T> result = new ArrayList<>();
        //遍历封装pid=0的
        for (T t : list) {
            //根据反射，获取父id的值
            ...
            if (ROOT_ID.equals(pId)) {
                ...
                T children = findChildren(t, list, idName, parentIdName, childrenName);
                result.add(children);
                break;
            }
        }
    ...
    public static <T> List<T> getTreeRole(List<T> list, String idName, String parentIdName, String childrenName) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        List<T> result = new ArrayList<>();
        ...
        for (T t : list) {
            ...
            if (ROOT_ID.equals(pId)) {
                ...
                T children = findChildren(t, list, idName, parentIdName, childrenName);
                String type = children.getClass().getMethod(GET + "Type").invoke(children).toString();
